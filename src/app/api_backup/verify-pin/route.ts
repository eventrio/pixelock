...
export async function POST(req: NextRequest) {
  ...
  const imgQ = await supabaseServer
    .from("images")
    .select("id, views_used, max_views, expires_at")
    .eq("token", token)
    .maybeSingle();
  if (imgQ.error || !imgQ.data) return NextResponse.json({ error: "Not found" }, { status: 404 });

  const img = imgQ.data;
  if (img.expires_at && new Date(img.expires_at) <= new Date()) {
    return NextResponse.json({ error: "Expired" }, { status: 410 });
  }
  if ((img.views_used ?? 0) >= img.max_views) {
    return NextResponse.json({ error: "Expired" }, { status: 410 });
  }

  // Get share row for attempts + pin hash
  const shareQ = await supabaseServer
    .from("image_shares")
    .select("id, pin_hash, attempt_count, locked_until")
    .eq("image_id", img.id)
    .maybeSingle();
  ...
  // Lockout?
  const now = new Date();
  if (share.locked_until && new Date(share.locked_until) > now) {
    return NextResponse.json({ error: "Locked. Try later." }, { status: 429 });
  }
  ...
  if (!ok) {
    ...
    if (attempts >= maxAttempts) {
      patch.locked_until = new Date(Date.now() + lockMins * 60 * 1000).toISOString();
    }
  ...
  // *** Consume a view immediately on successful PIN ***
  const newViews = (img.views_used ?? 0) + 1;
  const updImg = await supabaseServer
    .from("images")
    .update({ views_used: newViews })
    .eq("id", img.id)
    .select("views_used, max_views")
    .single();

  if (updImg.error) {
    return NextResponse.json({ error: "View update failed" }, { status: 500 });
  }
  if (updImg.data.views_used > updImg.data.max_views) {
    return NextResponse.json({ error: "Expired" }, { status: 410 });
  }

  // Create a viewing session with remaining time budget
  const remainingMs = 1000 * Number(process.env.VIEW_SECONDS || 15);
  const sessIns = await supabaseServer
    .from("view_sessions")
    .insert({
      image_id: img.id,
      remaining_ms: remainingMs,
      // expires_at default (10 min) is set in table
    })
    .select("id")
    .single();

  if (sessIns.error || !sessIns.data) {
